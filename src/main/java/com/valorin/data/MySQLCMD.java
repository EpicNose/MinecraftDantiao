...
public enum MySQLCMD {
	CREATE_TABLE_ARENA(
			...
			+ "`watchingpoint` LONGBLOB) DEFAULT CHARACTER SET = utf8;"),
	...
	CREATE_TABLE_POINTSHOP(
			...
			+ "`broadcast` TEXT,"
			+ "`description` TEXT,"
			+ "`salesvolume` INT) DEFAULT CHARACTER SET = utf8;"),
	...
	CREATE_TABLE_ENERGY("CREATE TABLE IF NOT EXISTS `dantiao_energy` ("
			+ "`name` TEXT,"
			+ "`energy` DOUBLE) DEFAULT CHARACTER SET = utf8;"); //result: 0为胜利 1为失败 2为平局

	private String command;
	
	MySQLCMD(String command) { this.command = command; }
